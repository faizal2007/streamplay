...
Usage (examples):
  ...
  python bench_simulator.py --mode http --url https://example.com --concurrency 10 --duration 30

  # playwright mode (real browser load; requires playwright installed & browsers)
  python bench_simulator.py --mode playwright --url https://example.com --concurrency 4 --duration 60 --screenshot-interval 2

You can also run inside the container built by the simulator service:
  docker-compose build simulator
  docker-compose run --rm simulator python bench_simulator.py --mode http --url https://example.com --concurrency 10 --duration 30
"""
import argparse
import asyncio
import base64
...
def estimate_for_target(stats: Dict[str, Any], target_concurrency: int, safety_factor: float = 1.5, baseline_os_mem_bytes: int = 512 * 1024 * 1024, retention_days: int = 7, runs_per_day: int = 24):
    ...
    cpu_cores = max(1, int(cpu_needed + 0.999))
    ...
    disk_needed_bytes = disk_needed_bytes + (10 * 1024 * 1024)
    return {
